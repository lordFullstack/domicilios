...
interface ProductImageProps {
  ...
  /**
   * Ancho real al que se muestra la foto (px). Se usa para pedirle a
   ...
   * que el hero. Si no se pasa, solo se aplica compresión (quality).
   */
  width?: number
}
...
export const ProductImage = ({ imageUrl, alt, className, emojiClassName, fallbackIconSize = 28, width }: ProductImageProps) => {
  ...
  if (isRealPhoto && failedUrl !== imageUrl) {
    ...
    const src = supabaseImageUrl(imageUrl, { width: width ? width * 2 : undefined })
    return (
