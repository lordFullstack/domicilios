-- LOOP_CLIENT_05C · M7 — create_order con p_notes_to_restaurant
--
-- REQUIERE M6 (orders.notes_to_restaurant) y M5 (p_cash_amount).
--
-- La firma pasa de 7 a 8 parámetros (p_notes_to_restaurant text default null). Se ELIMINA la de 7
-- y se crea la nueva con los mismos permisos (authenticated + service_role, nunca anon/public).
-- Una app en caché con 6 o 7 argumentos (por nombre) sigue resolviendo por los defaults.
--
-- Validación de las notas (D4), en el servidor y ANTES de insertar:
--   * trim; si queda vacía -> null.
--   * más de 150 caracteres -> se RECHAZA con notes_too_long (no se recorta en silencio).
-- Conserva intactas las validaciones de cash_amount de M5 y la idempotencia de M3.
...
create or replace function public.create_order(
  ...
  p_notes_to_restaurant text default null
)
...
declare
  ...
  v_notes text := nullif(trim(coalesce(p_notes_to_restaurant, '')), '');
begin
  ...
  if char_length(coalesce(v_notes, '')) > 150 then
  ...
  begin
    insert into public.orders (
      ...
      delivery_address, special_instructions, payment_method, client_order_id, cash_amount, notes_to_restaurant
    ) values (
      ...
      p_payment_method, p_client_order_id, p_cash_amount, v_notes
    )
...
revoke all on function public.create_order(uuid, text, text, text, jsonb, uuid, integer, text) from public, anon;
grant execute on function public.create_order(uuid, text, text, text, jsonb, uuid, integer, text) to authenticated, service_role;
