-- ROLLBACK de M3: vuelve a la función de 5 parámetros tal como estaba (verificada en la base, 24-sep-2026).
...
revoke all on function public.create_order(uuid, text, text, text, jsonb) from public, anon;
grant execute on function public.create_order(uuid, text, text, text, jsonb) to authenticated, service_role;
